...
def test_valkey_query_user_expected_output():
    assert valkey_query_user('testuser') == ['x.y.z', 'a.b.&', '1.2.&&']

def test_valkey_query_perm_expected_output():
    assert valkey_query_perm('x.y.z') == ['testuser']
    assert valkey_query_perm('a.b.&') == ['testuser']
    assert valkey_query_perm('1.2.&&') == ['testuser']
    
def test_valkey_check_expected_output_and_globbing():
    # Test expected output for permission 'x.y.z'
    assert valkey_check('testuser', 'x.y.z')
    assert valkey_check('testuser', 'x.y.z.a') == False

    # Test expected output for permission 'a.b.*'
    assert valkey_check('testuser', 'a.b.c')
    assert valkey_check('testuser', 'a.b.q')
    assert valkey_check('testuser', 'a.b.c.d') == False

    # Test expected output for permission '1.2.**'
    assert valkey_check('testuser', '1.2.3')
    assert valkey_check('testuser', '1.2.3.4')
    assert valkey_check('testuser', '1.2') == False
